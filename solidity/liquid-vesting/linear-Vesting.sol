use anchor_lang::{ prelude::*, solana_program::program_error, solana_program::clock };
use anchor_spl::token::{self, Mint, SetAuthority, TokenAccount, Transfer};
use spl_token::instruction::AuthorityType;

declare_id!("2DjqrTXfyxAvPwM9ieKcUt3rhEqTXrmDAoz95T7fhR3e");
...
pub mod linear_vesting {
    ...
    pub fn revoke(
        ...
        if !ctx.accounts.owner.is_signer {
            return Err(ProgramError(ProgramErrorWithOrigin::from(program_error::ProgramError::MissingRequiredSignature)))
        }
        if ctx.accounts.owner.key != &ctx.accounts.vesting_account.owner {
            return Err(LinerVestingError::NoMatchOwner.into());
        }
        if !ctx.accounts.vesting_account.revocable {
            return Err(LinerVestingError::NoRevoke.into());
        }
        if ctx.accounts.vesting_account.revoked {
            return Err(LinerVestingError::AlreadyRevoked.into());
        }
        ...
        if current_time < ctx.accounts.vesting_account.start_ts + ctx.accounts.vesting_account.cliff_ts {
            unreleased_amount = 0;
        } else if current_time >= ctx.accounts.vesting_account.start_ts + ctx.accounts.vesting_account.duration ||
            ctx.accounts.vesting_account.revoked {
            unreleased_amount = current_balance;
        }else{
            unreleased_amount = ((total_balance * (current_time - ctx.accounts.vesting_account.start_ts) as u64)
                / ctx.accounts.vesting_account.duration as u64)
                - ctx.accounts.vesting_account.released_amount;
        }
        let refund = current_balance - unreleased_amount;
        ctx.accounts.vesting_account.revoked = true;
        let (_vault_authority, vault_authority_bump) = Pubkey::find_program_address(&[VAULT_AUTHORITY_PDA_SEED], ctx.program_id);
        let authority_seeds = &[&VAULT_AUTHORITY_PDA_SEED[..], &[vault_authority_bump]];

        token::transfer(ctx.accounts.into_transfer_to_owner_context().with_signer(&[&authority_seeds[..]]), refund)?;
        Ok(())
    }
    pub fn withdraw(
        ...
        if current_time < ctx.accounts.vesting_account.start_ts + ctx.accounts.vesting_account.cliff_ts {
            ...
            unreleased_amount = ((total_balance * (current_time - ctx.accounts.vesting_account.start_ts) as u64) / ctx.accounts.vesting_account.duration as u64) - ctx.accounts.vesting_account.released_amount;
        }
        if unreleased_amount <= 0 {
            return Err(LinerVestingError::NoTokens.into());
        }
        ctx.accounts.vesting_account.released_amount += unreleased_amount;
        let (_vault_authority, vault_authority_bump) = Pubkey::find_program_address(&[VAULT_AUTHORITY_PDA_SEED], ctx.program_id);
        let authority_seeds = &[&VAULT_AUTHORITY_PDA_SEED[..], &[vault_authority_bump]];
        token::transfer( ctx.accounts.into_transfer_to_beneficiary_context().with_signer(&[&authority_seeds[..]]), unreleased_amount);
        Ok(())
    }
